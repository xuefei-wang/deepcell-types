...
- **Backbone: torchvision ResNet-50** (`model.py:46-48`), default
  `model_size="resnet50"`.
- **Current master uses a CIFAR-style stem sized to 32×32 patches**: the ImageNet
  stem (7×7 stride-2 conv + max-pool) collapses a 32×32 input to 1×1 by `layer4`,
  so it is replaced by a single **3×3 stride-1 conv with no max-pool**
  (`model.py:54-57`); the spatial path becomes 32→32→16→8→4→1. The follow-up
  paper-faithful CellSighter branch switches to a 60×60 crop with the ImageNet
  stem by default.
...
- **50 epochs, Adam, constant learning rate `1e-3`, no scheduler.** The upstream
  ...
  do not step a scheduler (`run.py:362-367`).
- **Best epoch selected on validation macro-accuracy** (`run.py:393-418`);
  validation runs every `val_every_n_epochs` (default 10) plus
...
- **Neighbor pixel intensities are zeroed (self-mask).** Our shared patch
  ...
  but for CellSighter it is a deviation. The follow-up paper-faithful CellSighter
  branch restores a 60×60 crop/unmasked-neighbor ablation path so this deviation
...
- **Smaller spatial context on current master.** We use 32×32 patches
  (`training/config.py:78-79`); upstream extracts 128 px and feeds a 60 px model
  input, i.e. a larger neighborhood. The paper-faithful CellSighter follow-up
  branch changes the default crop/input size to 60×60.
- **Class-balancing sampler differs on current master.** This branch uses the
  DeepCell Types sqrt-inverse-frequency `FOVGroupedSampler` with a 1000-sample
  effective-count floor (default `use_weighted_sampler=True`,
  `deepcell_types/training/samplers.py:41-45`), the same scheme as the main
  model. Upstream uses a full-inverse-frequency `WeightedRandomSampler` over a
  per-class capped pool (more aggressive rare-class upweighting). The
  paper-faithful CellSighter follow-up branch switches the default to the
  equal-proportion/full-inverse-frequency scheme.
